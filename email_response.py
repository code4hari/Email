...
def get_sent_emails(service):
    ...
    query = "in:sent"  # Removed date threshold to get all sent emails
...
def has_response(service, thread_id):
    ...
    return len(thread.get('messages', [])) > 1  # More than one message indicates a reply.
...
def generate_follow_up_email(chain_text):
    ...
    prompt = (
        ...
        f"---\n{chain_text}\n---\n"
        f"The follow-up should be concise, professional, and encourage a response. No work should be left to the user. The email should be complete in the sense it can be sent immedietly and containing no blanks for the user to fill in."
    )
...
def process_sent_emails(service):
    ...
    for email_data in emails:
        ...
        if not has_response(service, email['threadId']):  # Use threadId instead of email ID
...
if __name__ == "__main__":
    ...
    print("Checking all sent emails without a response...")
